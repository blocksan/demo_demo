...
<div class="information-blocks dynamicloading">
    ...
    <div class="cart-box popup">
        <?php 
            ...
            ?> 
<!-- <div class="popup-container cart-item-container"> -->
         <!-- <div class='cart-entry'>
                                <a class='image'><img height='40px'src='img/product-menu-1_.jpg' alt='' /></a>
                                <div class='content'>
                                    <a class='title' href='#'>$product_name</a>
                                    <div class='quantity'>$product_cat</div>
                                    <div class='price'>$990,00</div>
                                </div>
                            <div class='button-x discard-product' ><i class='fa fa-rupee' id='$product_id'></i></div>
            </div> -->
        </div>
    </div>
...
<!--                                         <a class="button style-10 add-to-cart" href="#">Add to cart</a> -->
 ...
 <!-- Pop Up to show different processed task -->
    ...
    //end init swiper
                    ...
                    $('.dynamicloading .parentselecter').load("loadingproducts2.php", {'page':'true'}, function(data) {
                        track_load++;
                         // alert(track_load);
                     ...
                     $('#loadmore').hide();
                               ...
                               if(track_load <=total_pages)
                                    {   $('.animation_loading_image').show(); 
                                        $.post('loadingmoreproducts2.php',{'page': track_load}, function(data){
...
/*$('.parentselecter_data').on('click','.des-entry',function(){
   // alert("hello");
  //$(this).closest('.product-slide-entry').hide();
    $(this).parents('.product-image').addClass('hover-class-1');
    $(this).parents('.product-image').children('img#1im').attr('src','img/loader.gif');
    $(this).parents('.product-image').children('img#2in').css({'z-index':'1000','opacity':'0.7'});

    $(this).parents('.product-image').children('img#2im').attr('src','img/spinningwheel.gif');

    $(this).parents('.bottom-line').children('.right-align').show();
    $(this).parents('.bottom-line').children('.left-align').show();

});
*/


$('.parentselecter_data').on('mouseenter','#1im',function()
 {
             var val = $(this).siblings('.design_id').attr('value');
             $('.product-image').find('.load_option').css({"display":"none"});
             $(this).parents('.product-image').find('.load_option').css({"display":"block"});
             $.post('loadingdesigntypes.php',{des:val}, function(data)
                                    {
                                       showDesign(data);
                                       /*var parsedData = jQuery.parseJSON(data);
     ...
     function showDesign(data)
           {
            $('.product-image').find('.hoodie').css({"display":"none"});
            $('.product-image').find('.half_tshirt').css({"display":"none"});
            $('.product-image').find('.full_tshirt').css({"display":"none"});
            $('.product-image').find('.zipper').css({"display":"none"});
            $('.product-image').find('.load_option').css({"display":"none"});
             var parsedData = jQuery.parseJSON(data);
                                        var i=0;
                                        var proID='',proID1='',proID2='',proID3='' ;
                                        for(i=0;i<=parsedData.count;i++)
                                        {        if(parsedData[i].type_name==='hoodie')   
                                                {   
                                                    console.log(parsedData[i].product_id+'hoodie');
                                                    $('.product-image').find('.hoodie').css({"display":"block","float":"left"});
                                                     proID=parsedData[i].product_id;
                                                     $('.product-image').find('.hoodie').on('click',this,function(){
                                                        
                                                        console.log(proID+" ii0");
                                                    });
                                                 }
                                                else if(parsedData[i].type_name==='full_tshirt')
                                                {
                                                    $('.product-image').find('.full_tshirt').css({"display":"block","float":"left"});
                                                    console.log(parsedData[i].product_id+'full_tshirt');
                                                    proID1=parsedData[i].product_id;
                                                    $('.product-image').find('.full_tshirt').on('click',this,function(){
                                                        
                                                        console.log(proID1+" ii1");
                                                    });
                                                }
                                                else if(parsedData[i].type_name==='half_tshirt')
                                                {   $('.product-image').find('.half_tshirt').css({"display":"block","float":"left"});
                                                    console.log(parsedData[i].product_id+'half_tshirt');
                                                    proID2=parsedData[i].product_id;
                                                    $('.product-image').find('.half_tshirt').on('click',this,function(){
                                                        
                                                        console.log(proID2+" ii2");
                                                    });      
                                                  }
                                                else if(parsedData[i].type_name==='zipper')
                                                {   $('.product-image').find('.zipper').css({"display":"block","float":"left"});
                                                    console.log(parsedData[i].product_id+'zipper');
                                                    proID3=parsedData[i].product_id;
                                                    $('.product-image').find('.zipper').on('click',this,function(){
                                                        
                                                        console.log(proID3+" ii3");
                                                    });
                                                }
